...
public class BiomeFeatures {
    ...
    public static final HugeTreeFeatureConfig mega_darkwood_tree = new HugeTreeFeatureConfig.Builder(new SimpleBlockStateProvider(BlockRegistry.darkwood_log.getDefaultState()), new SimpleBlockStateProvider(BlockRegistry.darkwood_leaves.getDefaultState())).baseHeight(13).heightInterval(15).crownHeight(13).setSapling(((IPlantable)BlockRegistry.darkwood_sapling)).build();
...
/*    @SubscribeEvent
    ...
    public static void addDarkwoodTrees(Biome biomeIn) {
        biomeIn.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Feature.RANDOM_SELECTOR.withConfiguration(new MultipleRandomFeatureConfig(ImmutableList.of(Feature.MEGA_SPRUCE_TREE.withConfiguration(mega_darkwood_tree).withChance(0.5F), Feature.NORMAL_TREE.withConfiguration(darkwood_tree).withChance(0.5F)), Feature.NORMAL_TREE.withConfiguration(darkwood_tree))).withPlacement(Placement.COUNT_EXTRA_HEIGHTMAP.configure(new AtSurfaceWithExtraConfig(12, 0.3F, 3))));
    }

    public static void addDarkwoodShrubs(Biome biomeIn) {
        biomeIn.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Feature.JUNGLE_GROUND_BUSH.withConfiguration(darkwood_bush).withPlacement(Placement.COUNT_EXTRA_HEIGHTMAP.configure(new AtSurfaceWithExtraConfig(5, 0.3F, 2))));
    }
}