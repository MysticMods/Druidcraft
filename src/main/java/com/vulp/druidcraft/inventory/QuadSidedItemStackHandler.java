...
public class QuadSidedItemStackHandler extends ItemStackHandler {
    ...
    public QuadSidedItemStackHandler(int size) {
    ...
    public QuadSidedItemStackHandler(NonNullList<ItemStack> inv1, NonNullList<ItemStack> inv2, NonNullList<ItemStack> inv3, NonNullList<ItemStack> inv4) {
        this.inv1 = inv1;
        this.inv2 = inv2;
        this.inv3 = inv3;
        this.inv4 = inv4;
    ...
    public int getRemainderInv(int slot) {
        if (slot >= this.inv1.size()) {
            if (slot >= this.inv2.size() + this.inv1.size()) {
                if (slot >= this.inv3.size() + this.inv2.size() + this.inv1.size()) {
                    return slot - this.inv1.size() + this.inv2.size() + this.inv3.size();
                }
                return slot - this.inv1.size() + this.inv2.size();
            }
    ...
    @Override
    public int getSlots() {
        return inv1.size() + inv2.size() + inv3.size() + inv4.size();
    }
    ...
    public ItemStack getStackInSlot(int slot) {
        ...
        if (slot >= this.inv1.size()) {
            if (slot >= this.inv2.size() + this.inv1.size()) {
                if (slot >= this.inv3.size() + this.inv2.size() + this.inv1.size()) {
                    return this.inv4.get(slot - (this.inv1.size() + this.inv2.size() + this.inv3.size()));
                } return this.inv3.get(slot - (this.inv1.size() + this.inv2.size()));
    ...
    public ItemStack insertItem(int slot, @Nonnull ItemStack stack, boolean simulate) {
        ...
        if (slot >= this.inv1.size()) {
            if (slot >= this.inv2.size() + this.inv1.size()) {
                if (slot >= this.inv3.size() + this.inv2.size() + this.inv1.size()) {
                    existing = this.inv4.get(slot - (this.inv1.size() + this.inv2.size() + this.inv3.size()));
