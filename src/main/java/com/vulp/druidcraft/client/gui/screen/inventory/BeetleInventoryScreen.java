...
public class BeetleInventoryScreen extends ContainerScreen<BeetleInventoryContainer> {
    ...
    protected void drawGuiContainerForegroundLayer(MatrixStack matrixStack, int x, int y) {
        this.font.drawString(matrixStack, this.title.getString(), 18.0F, 6.0F, 4210752);
        this.font.drawString(matrixStack, this.playerInventory.getDisplayName().getString(), 49.0F, 144.0F, 4210752);
    }
    ...
    protected void drawGuiContainerBackgroundLayer(MatrixStack matrixStack, float partialTicks, int mouseX, int mouseY) {
        ...
        if (this.beetleEntity instanceof BeetleEntity) {
            ...
            if (beetleEntity.hasChest()) {
                for (int k = 0; k < 7; k++) {
                    this.blit(matrixStack, i + 84, j + 17 + (k * 18), 0, this.ySize, 162, 18);
                }
            }
        }
