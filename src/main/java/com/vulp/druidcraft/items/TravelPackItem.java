...
public class TravelPackItem extends Item {
        ...
        if(world != null) {
            ...
            if (entity instanceof PlayerEntity) {
                ...
                Direction direction = player.getHorizontalFacing();
                    ...
                    if (sleep == IForgeDimension.SleepResult.DENY) {
                        player.sendStatusMessage(new TranslationTextComponent("block.druidcraft.bedroll.fail.dimension"), true);
                        return;
                    }
                    if (sleep == IForgeDimension.SleepResult.BED_EXPLODES) {
                        player.sendStatusMessage(new TranslationTextComponent("block.druidcraft.bedroll.fail.explode"), true);
                        return;
                    }
    ...
    public static void bedrollSleepHelper(BlockState state, World worldIn, BlockPos pos, PlayerEntity player) {
        ((BedrollBlock)state.getBlock()).onBlockActivated(state, worldIn, pos, player, Hand.MAIN_HAND, new BlockRayTraceResult(new Vec3d(0.0d, 0.0d, 0.0d), Direction.NORTH, pos, true));
    }
